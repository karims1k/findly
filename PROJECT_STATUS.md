...
## NEXT TASKS
1. **Re-add `http://localhost:3000/**` to Supabase Auth → URL Configuration → Redirect URLs.** It was dropped when the list was updated for the custom domain (only the two `findlybeauty.com` URLs and the `*.vercel.app` one remain) — local dev magic-link sign-in will fail until it's back. Low urgency (doesn't affect production), but will bite whoever next tests auth locally.
...
- **Magic-link sign-in on the new domain is confirmed working** — Supabase's Auth redirect URLs now include `findlybeauty.com`/`www.findlybeauty.com`, Site URL was updated to match, and the user verified a real sign-in end-to-end on `https://findlybeauty.com`. Remaining gap: `http://localhost:3000/**` was dropped from the Redirect URLs list in the process — see NEXT TASKS #1 (local dev only, doesn't affect production).
