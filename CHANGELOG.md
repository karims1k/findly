...
## 2026-09-02 — Shared Redis cache for /api/compare (pending push)
...
- **Notes**: Reads two possible env var name pairs — `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN` or `KV_REST_API_URL`/`KV_REST_API_TOKEN` — since Vercel's Upstash marketplace integration can surface either. User connected the Upstash integration in the Vercel dashboard; this entry will get a real commit hash once pushed.
